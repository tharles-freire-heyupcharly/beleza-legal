...
import com.mackenzie.belezalegal.databinding.FragmentQuestion11Binding
...
class Question11Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer11.checkedRadioButtonId == -1)
            ){
                findNavController().navigate(R.id.action_question11Fragment_to_question12Fragment)
            } else{
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer11,resources.getStringArray(R.array.answer11))
        binding.answer11.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer11.value = rb.text.toString()
            viewModel.weight11.value = index.toString()
        }
    }
}
