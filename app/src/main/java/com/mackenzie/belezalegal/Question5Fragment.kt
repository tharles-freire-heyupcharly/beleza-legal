...
import com.mackenzie.belezalegal.databinding.FragmentQuestion5Binding
...
class Question5Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer5.checkedRadioButtonId == -1)
            ){
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer5,resources.getStringArray(R.array.answer5))
        binding.answer5.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer5.value = rb.text.toString()
            viewModel.weight5.value = index.toString()
        }
    }
}
