...
import com.mackenzie.belezalegal.databinding.FragmentQuestion10Binding
...
class Question10Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer10.checkedRadioButtonId == -1)
            ){
                viewModel.saveToSheet(context)
                findNavController().navigate(R.id.action_question10Fragment_to_handoutInformalFragment)
            } else{
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer10,resources.getStringArray(R.array.answer10))
        binding.answer10.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer10.value = rb.text.toString()
            viewModel.weight10.value = index.toString()
        }
    }
}
