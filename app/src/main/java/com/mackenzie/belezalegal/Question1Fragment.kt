...
import com.mackenzie.belezalegal.databinding.FragmentQuestion1Binding
...
class Question1Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer1.checkedRadioButtonId == -1)
            ){
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer1,resources.getStringArray(R.array.answer1))
        binding.answer1.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer1.value = rb.text.toString()
            viewModel.weight1.value = index.toString()
        }
    }
}
