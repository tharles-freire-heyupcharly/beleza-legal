...
import com.mackenzie.belezalegal.databinding.FragmentQuestion4Binding
...
class Question4Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer4.checkedRadioButtonId == -1)
            ){
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer4,resources.getStringArray(R.array.answer4))
        binding.answer4.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer4.value = rb.text.toString()
            viewModel.weight4.value = index.toString()
        }
    }
}
