...
import com.mackenzie.belezalegal.databinding.FragmentQuestion7Binding
...
class Question7Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer7.checkedRadioButtonId == -1)
            ){
                findNavController().navigate(R.id.action_question7Fragment_to_question8Fragment)
            } else{
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer7,resources.getStringArray(R.array.answer7))
        binding.answer7.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer7.value = rb.text.toString()
            viewModel.weight7.value = index.toString()
        }
    }
}
