...
import com.mackenzie.belezalegal.databinding.FragmentQuestion6Binding
...
class Question6Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonNext.setOnClickListener {
            if(
                !(binding.answer6.checkedRadioButtonId == -1)
            ){
                findNavController().navigate(R.id.action_question6Fragment_to_question7Fragment)
            } else{
    ...
    private fun setFields(){
        val rb = fields.setRadioOptions(context,binding.answer6,resources.getStringArray(R.array.answer6))
        binding.answer6.setOnCheckedChangeListener { group, checkedId ->
            val rb = group.findViewById<RadioButton>(checkedId)
            val index = group.indexOfChild(rb)
            viewModel.answer6.value = rb.text.toString()
            viewModel.weight6.value = index.toString()
        }
    }
}
